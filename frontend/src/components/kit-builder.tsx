...
export default function KitBuilder({ course, csrf, editing = false, onSaved, onRegenerated }: { course: Course; csrf: string; editing?: boolean; onSaved: (course: Course) => void; onRegenerated: () => void }) {
  ...
  return <>
    {editing && <p className="save-status" role="status">{status}</p>}
    ...
    {editing ? <section className="panel kit-editor" aria-label="Course editor">
      <p className="field-help">Changes save automatically. Your additions, edits and pins are kept during regeneration. Hold Ctrl or Command to select multiple topics.</p>
      ...
      {tab === 'Questions' && <><button className="secondary" onClick={() => change(k => { k.questions.push({ id: `manual_${crypto.randomUUID()}`, prompt: '', answer_outline: '', category: 'technical', difficulty: 2, requirement_ids: [] }); })}>Add question</button>{draft.questions.map((q, i) => <article className="editor-item" key={q.id} aria-label={`Question ${i + 1}`}>
        ...
        <label>Category<select aria-label="Category" value={q.category} onChange={e => change(k => { k.questions[i].category = e.target.value; })}>{categories.map(category => <option key={category} value={category}>{categoryLabels[category]}</option>)}</select></label>
    ...
    {editing && <section className="panel regeneration-controls"><h3>Refresh one section</h3><p className="field-help">Your edits, pins, manual items and deletions are kept. Other sections stay as they are.</p><label>Section to regenerate<select aria-label="Section to regenerate" value={section} onChange={e => { setSection(e.target.value); requestKey.current = null; }}><option value="company_brief">Company brief</option><option value="schedule">Study schedule</option>{categories.filter(category => category !== 'system-design' || draft.questions.some(q => q.category === category)).map(category => <option key={category} value={`questions_${category}`}>{categoryLabels[category]} questions</option>)}</select></label><button className="secondary" disabled={regenerating || course.status === 'generating' || !!error} onClick={regenerate}>{regenerating || course.status === 'generating' ? 'Generation in progress…' : 'Regenerate section'}</button></section>}
  </>;
}
