...
const defaults: CourseInput = { title: '', company_name: '', company_url: '', jd: '', days: 14, daily_minutes: 120, availability_scope: 'shared' };
export default function CourseForm({ csrf, initial, onSaved }: { csrf: string; initial?: Course; onSaved: (course: Course) => void }) {
  const [values, setValues] = useState<CourseInput>(initial ? { title: initial.title, company_name: initial.company_name, company_url: initial.company_url, jd: initial.jd, days: initial.days, daily_minutes: initial.daily_minutes, availability_scope: initial.availability_scope } : defaults);
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (tab === 'batch') {
        ...
        const result = await api<{ results: { course: Course; created: boolean }[] }>('/courses/batch', { method: 'POST', body: JSON.stringify({ cases, daily_minutes: values.daily_minutes }) }, csrf);
        ...
        const result = await api<{ course: Course; created?: boolean }>(initial ? `/courses/${initial.id}` : '/courses', { method: initial ? 'PUT' : 'POST', body: JSON.stringify({ ...values, ...(initial ? { revision: initial.revision } : {}) }) }, csrf);
        setNotice(initial ? 'Changes saved.' : result.created ? 'Course saved. Open it from your course list.' : 'This course is already saved. You can open it from your course list.');
  ...
  return <section className="panel intake-panel"><div className="panel-heading"><div className="section-icon"><FileText size={19} /></div><div><h2>{initial ? 'Preparation details' : 'Start a new course'}</h2><p>{initial ? 'Keep your preparation inputs up to date.' : 'One opportunity. One focused plan.'}</p></div></div>
    ...
    <form onSubmit={submit}>
      ...
      <div className="time-section"><span className="small-heading"><Clock3 size={15} /> MAKE IT FIT YOUR DAY</span><div className="form-row">
      ...
      <button className="primary full" disabled={busy || extracting}>{extracting ? 'Reading PDF...' : busy ? 'Saving…' : initial ? 'Save changes' : tab === 'batch' ? 'Import courses' : 'Create course'}<ArrowRight size={17} /></button>
      <p className="form-footnote">{initial ? 'Your changes are securely saved to your account.' : 'Save your inputs, then start generation from your course.'}</p>
    </form>
  </section>;
}
